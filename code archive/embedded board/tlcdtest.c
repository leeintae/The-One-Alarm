/*
Below code is same with original code EXCEPT setDDRAMAddr() function.
*/
...
int setDDRAMAddr(int x , int y)
{
	...
	//To increase immediacy, we corrected below code.
	if ( x == 1 )// parameter x=1 means that we want to treat FIRST line of text LCD.
	{
		cmd = DDRAM_ADDR_LINE_1 + (y-1);
	}
	else if( x == 2 )// parameter x=2 means that we want to treat FIRST line of text LCD.
	{
		cmd = DDRAM_ADDR_LINE_2 + (y-1);
	}
	else
		return FALSE;
